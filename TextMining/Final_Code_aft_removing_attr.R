...
library("RTextTools")
library("stringr")
setwd("D:\\semester2\\BIS\\assignment3")
...
train_corpus <- c(alt.atheism.train,comp.graphics.train,
                  comp.windows.x.train,rec.autos.train,
                  rec.motorcycles.train,sci.electronics.train,rec.sport.hockey.train,
...
train_corpus_pre[[1]]
term.document.train <- DocumentTermMatrix(train_corpus_pre,control=list(minWordLength=2, minDocFreq=5))
...
#dataframe<-data.frame(text=unlist(sapply(train_corpus, '[')), stringsAsFactors=F)
#dim(dataframe)
#write.csv(dtm.sci.rel, "out.csv")
#dim(dtm.sci.rel)
#rownames(dtm.sci.rel)<- 1:nrow(dtm.mx.sci.rel)
class <- c(rep("alt.atheism",300), rep("comp.graphics",300), rep("comp.windows.x",300), 
           rep("rec.autos",300), rep("rec.motorcycles",300), 
           rep("sci.electronics",300), rep("rec.sport.hockey",300), 
...
test_data <- c(alt.atheism.test,comp.graphics.test,comp.windows.x.test, rec.autos.test,
               rec.motorcycles.test,sci.electronics.test,rec.sport.hockey.test,
...
dim(predictions)
class_test <- c(rep("alt.atheism",100), rep("comp.graphics",100), rep("comp.windows.x",100), 
                rep("rec.autos",100), rep("rec.motorcycles",100), 
                rep("sci.electronics",100), rep("rec.sport.hockey",100), 
